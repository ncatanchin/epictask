...
function init() {
	...
	inlineAttachment.prototype.uploadFile = function (file) {
		const
			{isDropboxReady,getDropboxClient} = require('epic-global/DropboxUtil')
		...
		if (file.name) {
			let
				fileNameMatches = file.name.match(/\.(.+)$/);
		...
		const
			uploadPath = `/images/markdown-editor/${remoteFilename}`
		...
		function handleError(err) {
			log.error(`Upload failed for ${uploadPath}`,err)
			me.onFileUploadError(err)
		...
		if (!isDropboxReady()) {
			handleError(new Error(`Dropbox account is not linked or invalid, to drag and drop as well as paste images please link a dropbox account in settings.`))
		} else {
			...
			fileReader.onload = event => {
				...
				debugger
				db.filesUpload({path: uploadPath, contents: data})
					.then(uploadResult => {
						
						log.debug(`Upload result for ${uploadPath}`, uploadResult)
						
						filename = uploadResult.path_lower
						
						
						return db.sharingCreateSharedLinkWithSettings({
							path: filename,
							settings: {
					...
					// SHARED LINK RESULT
					.then(shareResult => {
						log.debug(`Sharing result for ${uploadPath}/${filename}`, shareResult)
						
						const
							{url} = shareResult
						
						me.onFileUploadResponse({uploadPath: filename, url})
	 ...
	 * @return {Void}
	 */
	inlineAttachment.prototype.onFileUploadResponse = function (dbResult) {
		
		if (this.settings.onFileUploadResponse.call(this, dbResult) !== false) {
			let
				{uploadPath,url} = dbResult
	 ...
	 * @return {Void}
	 */
	inlineAttachment.prototype.onFileUploadError = function (err) {
		if (this.settings.onFileUploadError.call(this, err) !== false) {
			let text = this.editor.getValue().replace(this.lastValue, "");
			this.editor.setValue(text);
		}
	};
