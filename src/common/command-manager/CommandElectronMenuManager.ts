...
import {getCommandManagerType, getCommandManager, CommandManager} from "./CommandManager"
...
import {inElectron} from "common/ElectronUtil"
...
const
	...
	instanceContainer = getHot(module,'instanceContainer',{} as any) as {
 ...
 * @returns {boolean}
 */
function menuHasItems(menu:Electron.Menu|Electron.MenuItem[]|any[]):boolean {
...
export class CommandElectronMenuManager implements ICommandMenuManager {
	...
	internalShowItem(...items:ICommandMenuItem[]) {
		...
		// FORCE UPDATE
		const
			newAppMenu = new Electron.Menu()
		
		appMenu.items
			.filter((item:any) => item.type !== 'submenu' || menuHasItems(item.submenu))
	...
	private internalHideItem(...itemIds) {
		...
		try {
			const hideEmptyChildren = (checkMenu):void => {
				const
					items = Array.isArray(checkMenu) ? checkMenu : checkMenu.items
				
				items
					.forEach((item:Electron.MenuItem & any,index) => {
						if (item.submenu) {
...
export const getCommandElectronMenuManager = getHot(module,'getCommandElectronMenuManager',new Proxy(function(){},{
	apply: function(target,thisArg,args):CommandElectronMenuManager {
		return instanceContainer.clazz.getInstance()
	}
}) as any) as () => CommandElectronMenuManager
 ...
 */
function getMainCommandManager():CommandManager {
...
export const ElectronMainManagerProvider = ():CommandElectronMenuManager => {
	return 	(isMain()) ?
		getCommandElectronMenuManager() :
		getMainCommandManager().getMenuManager() as any
}
 ...
 */
if (DEBUG)
	Object.assign(global,{getCommandElectronMenuManager})
